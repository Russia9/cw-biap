...
* Computes the freestream state (sea-level ISA by default), the turbulence inlet
  ...
  Mach (flat-plate estimate; refine with the yPlus function object and rerun).
...
def main() -> None:
    ...
    ap.add_argument("--layers", type=int, default=6, help="boundary-layer cell count")
    ap.add_argument("--expansion", type=float, default=1.2, help="layer expansion ratio")
    ...
    # 3. y+ first-layer height (flat-plate Cf at mid-body) -------------------
    Re = rho * U * (L / 2.0) / mu
    Cf = (2.0 * math.log10(Re) - 0.65) ** -2.3          # Schlichting
    utau = U * math.sqrt(Cf / 2.0)
    y_centroid = args.yplus * mu / (rho * utau)
    first_layer = 2.0 * y_centroid                       # centroid sits at y_centroid

    # 4. domain + refinement, scaled from the part bbox ----------------------
    tok = {
        "xIn": fmt(-5.0 * L), "xOut": fmt(11.0 * L),
        "Rfar": fmt(5.0 * L), "c": fmt(5.0 * L / math.sqrt(2.0)),
        "a": fmt(5.1 * R),
        "bcX1": fmt(-0.437 * L), "bcX2": fmt(3.0 * L), "bcR": fmt(7.6 * R),
        "nbX1": fmt(-0.164 * L), "nbX2": fmt(1.64 * L), "nbR": fmt(3.16 * R),
        "cbX1": fmt(-0.0547 * L), "cbX2": fmt(1.148 * L), "cbR": fmt(1.65 * R),
        "locX": fmt(-2.7 * L), "locYZ": fmt(1.5 * R),
        "firstLayer": fmt(first_layer), "nLayers": str(args.layers),
        "expansion": fmt(args.expansion),
        "np": str(args.np), "solver": solver,
    }
    ...
    # shared static dicts
    copy(common / "constant/thermophysicalProperties", out / "constant/thermophysicalProperties")
    copy(common / "constant/turbulenceProperties", out / "constant/turbulenceProperties")
    copy(common / "system/surfaceFeatureExtractDict", out / "system/surfaceFeatureExtractDict")
    ...
    write_freestream(out / "constant/freestreamProperties", args, U, Ux, Uz,
                     rho, q, kInf, omegaInf, Aref, L_all, dragDir, liftDir)
    ...
    print(f"  part={args.part}  Ma={args.Ma}  alpha={args.alpha} deg  "
          f"p={fmt(args.p)} Pa  T={fmt(args.T)} K")
    print(f"  geometry: L={fmt(L)} m  R_body={fmt(R)} m   "
          f"(reference part=all: L={fmt(L_all)} m  Aref={fmt(Aref)} m^2)")
    print(f"  flow:     a={fmt(a)}  |U|={fmt(U)} m/s  rho={fmt(rho)}  "
          f"mu={fmt(mu)}  Re_mid={Re:.3e}")
    print(f"  turb:     kInf={fmt(kInf)}  omegaInf={fmt(omegaInf)}")
    print(f"  layers:   firstLayerThickness={first_layer:.3e} m  "
          f"nLayers={args.layers}  expansion={fmt(args.expansion)}  (target y+={fmt(args.yplus)})")
    print(f"  decompose: {args.np} subdomains\n"
          f"  next: cd {out} && ./Allrun.pre   # mesh, then check yPlus and rerun if y+ drifts")


def write_freestream(path, args, U, Ux, Uz, rho, q, kInf, omegaInf,
                     Aref, lRef, dragDir, liftDir) -> None:
    def vec(t):
