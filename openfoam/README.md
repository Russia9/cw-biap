...
> table. With no result CSVs at all it exits before writing, so that case is
> safe; with some, redirect via `--summary <scratch>/averages.csv`.

## Coefficient reference

`forceCoeffs` uses a **single fixed reference taken from part=all** — frontal
area `Aref = π·R_all²` (1.96067 m²) and `lRef = L_all` (18.243 m), moment center at
