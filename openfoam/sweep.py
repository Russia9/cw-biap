...
SWEEP: dict[str, dict[str, list[tuple[float, int]]]] = {
    "all": {
        ...
        "alpha": [(0, 1), (2, 1), (4, 0), (6, 1), (8, 0), (10, 0)],
    },
}
...
def set_status(state: dict, path: Path, case: Case, base: Path, **fields) -> None:
    ...
    rec.update(fields)
    rec["updated"] = _dt.datetime.now().astimezone().isoformat(timespec="seconds")
...
def main() -> None:
    ...
    for i, c in enumerate(todo, 1):
        ...
        try:
            ...
            n_fail += 1
        except Exception as e:  # noqa: BLE001 - never let one case kill the sweep
