...
def scad_params(geo: list[Geometry], sub: Subrockets) -> str:
    """Render rocket-params.scad: the geometry rocket.scad takes from this
    script, rounded to the precision the report tables quote so the drawing,
    the document and the CFD reference area all agree digit for digit.

    Only the motor-derived numbers live here. The payload, nose and interstage
    dimensions are CAD choices this script does not compute, and stay in
    rocket.scad."""

    def row(name: str, values: list[float], spec: str, comment: str) -> str:
        cells = ", ".join(f"{v:{spec}}" for v in values)
        return f"{name:<5}= [{cells}];  // {comment}"
    ...
    return "\n".join(
        [
            ...
            "",
            row("d_m", sub.d_m, ".2f", 'motor-case diameter  d_(м i)'),
            row("L", [g.L for g in geo], ".2f", "overall stage length L_i"),
            "",
            "// Nozzles are not modelled (they broke snappyHexMesh); these are",
            "// carried for the echo block and drawing reference only.",
            row("d_kr", [g.d_kr for g in geo], ".3f", 'nozzle throat dia    d_("кр" i)'),
            row("d_a", [g.d_a for g in geo], ".3f", "nozzle exit dia      d_(a i)"),
            row("l_a", [g.l_a for g in geo], ".3f", "nozzle bell length   l_(a i)"),
            "",
        ]
    )


def sync_scad_params(geo: list[Geometry], sub: Subrockets, write: bool) -> None:
    """Keep rocket-params.scad in sync with this script.

    The file is generated whole, so drift detection is a plain text comparison
    against a freshly rendered copy — there is no need to parse .scad back.

    Without ``write`` the file is only checked, and any drift is reported on
    stderr so that piping stdout into the Typst report stays unaffected.

    With ``write`` the file is rewritten even when it already matches, so that
    its mtime advances past main.py's — the Makefile rebuilds rocket-params.scad
...
def main():
    ...
    geo = emit_geometry(sub.d_m)
    sync_traj_config(thrust, sub, write="--write-traj-config" in sys.argv)
    sync_scad_params(geo, sub, write="--write-scad-params" in sys.argv)
